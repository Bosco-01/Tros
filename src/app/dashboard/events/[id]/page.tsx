...
export default function EventDetailsPage({ params }: PageProps) {
  ...
  useEffect(() => {
    const fetchEventDetails = async () => {
      try {
        ...
        const reviewCountVal = (r.review_count ?? r.reviews_count ?? r.reviewsCount ?? r.reviewCount) as unknown;
        ...
        const mapped = {
          ...
          vendorName: (r.vendor_name as string) || (r.vendorName as string) || '',
          totalUsers: r.total_users ? String(r.total_users) : (r.totalUsers as string) || '0',
          price: typeof r.price === 'number' ? `# ${Number(r.price).toLocaleString()}` : (r.price as string) || '',
          dateTime: (r.date_time as string) || (r.dateTime as string) || '',
          workingHours: (r.working_hours as string) || (r.workingHours as string) || undefined,
          status: ((r['status'] as string) || 'active'),
          rating: typeof ratingVal === 'number' ? (ratingVal as number) : 0,
          reviewsCount: typeof reviewCountVal === 'number' ? (reviewCountVal as number) : Number(reviewCountVal as string) || 0,
          bannerUrls: Array.isArray(r.banner_urls) ? (r.banner_urls as string[]) : (r.cover_image_url ? [String(r.cover_image_url)] : []),
        ...
        console.warn('Failed to fetch live event details. Falling back to simulation data.', err);
  ...
  const handleApproveCancellation = async () => {
    if (!confirm('Are you sure you want to approve this vendor cancellation request? This will automatically refund all booked users.')) return;
            ...
            <div className="w-full bg-[#FFE8E8] border border-[#D82F2F]/20 rounded-3xl p-6 mb-8 flex flex-col sm:flex-row items-center justify-between gap-4 select-none animate-in fade-in duration-300">
              <div className="flex items-center gap-3.5">
                ...
                <div className="flex flex-col text-left">
                  <h4 className="text-[16px] font-bold text-neutral-950">Pending Vendor Cancellation Request</h4>
                  <p className="text-xs font-semibold text-neutral-600 mt-1 max-w-lg leading-relaxed">
                    This vendor has requested to cancel this event. Approving this request will remove the listing and trigger automatic user refunds.
                  </p>
                </div>
              </div>
              ...
              >
                {approvingCancellation ? 'Processing...' : 'Approve Cancellation'}
              </button>
            </div>
          )}
