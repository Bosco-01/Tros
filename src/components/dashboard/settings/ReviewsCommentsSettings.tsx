  ...
  useEffect(() => {
    const loadSettings = async () => {
      try {
        ...
        console.warn('[Settings] Failed to fetch live reviews settings. Falling back to local default values.');
      } finally {
        setIsLoading(false);
      }
    };
    ...
    <form onSubmit={handleSave} className="bg-white rounded-3xl p-8 border border-neutral-100 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.03)] flex-1 w-full flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-4 w-full">
        <ToggleCard
          ...
          description="Allow users to leave reviews on events"
          field="enableReviews"
