...
const getCategoryStyles = (category: 'Nightlife' | 'Music' | 'Hotel') => {
  switch (category) {
    case 'Nightlife':
...
const getStatusStyles = (status: 'Active' | 'Pending' | 'Declined') => {
  switch (status) {
    case 'Active':
  ...
  return (
    <div className="w-full bg-white rounded-3xl border border-neutral-100 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] overflow-hidden flex flex-col max-w-[1100px]">
        ...
        <table className="w-full text-left border-collapse min-w-[950px]">
          <thead>
            <tr className="bg-[#F8F9FA] border-b border-neutral-100">
              <th className="px-6 py-5 text-sm font-bold text-neutral-900">Category</th>
              <th className="px-6 py-5 text-sm font-bold text-neutral-900">Title</th>
              <th className="px-6 py-5 text-sm font-bold text-neutral-900">Event Type</th>
              <th className="px-6 py-5 text-sm font-bold text-neutral-900">Price</th>
              <th className="px-6 py-5 text-sm font-bold text-neutral-900">Date and Time</th>
              <th className="px-6 py-5 text-sm font-bold text-neutral-900">Vendor name/ ID</th>
            ...
            {data.map((row, index) => (
              ...
              >
                ...
                <td className="px-6 py-5">
                  <span className={`inline-flex px-3 py-1.5 rounded-lg text-xs font-bold leading-none ${getCategoryStyles(row.category)}`}>
                    {row.category}
                  </span>
                </td>

                <td className="px-6 py-5 text-[15px] text-neutral-600 font-medium">{row.title}</td>
                <td className="px-6 py-5 text-[15px] text-neutral-600 font-medium">{row.eventType}</td>
                <td className="px-6 py-5 text-[15px] text-neutral-600 font-bold">{row.price}</td>

                {/* Date & Time */}
                <td className="px-6 py-5">
                  <div className="flex flex-col gap-0.5">
                    <span className="text-[14px] font-bold text-neutral-950">{row.date}</span>
                ...
                <td className="px-6 py-5">
                  <div className="flex flex-col gap-0.5">
                    <span className="text-[14px] font-bold text-neutral-950">{row.vendorName}</span>
                    <span className="text-[13px] font-medium text-neutral-500">{row.vendorId}</span>
                  </div>
                </td>
      ...
      <div className="px-8 py-5 border-t border-neutral-100 flex items-center justify-between bg-white mt-auto select-none">
        <span className="text-[15px] font-semibold text-neutral-900">1 of 102</span>
