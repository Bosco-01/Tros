...
export const Sidebar = () => {
  ...
  const handleLogout = async () => {
    try {
      // Trigger the server-side cookie deletion
      await fetch('/api/auth/logout', { method: 'POST' });
    } catch (error) {
      console.error('Failed to log out cleanly:', error);
    } finally {
      // Force redirect back to login and reload page memory
  ...
  return (
    <aside className="w-[260px] flex-shrink-0 h-screen bg-white border-r border-neutral-100 flex flex-col pt-8 pb-6 sticky top-0 hidden lg:flex">
      {/* Brand Logo */}
      <div className="flex items-center gap-3 px-8 mb-10 select-none">
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-4 px-4 py-3.5 rounded-xl transition-colors ${
                isActive
                  ? 'text-neutral-900 font-bold bg-neutral-50/90'
